...
public class ScannerEx1 {
	...
	public static void main(String[] args) {
		...
		int num1 = sc.nextInt(); // 다음에 입력된 정수를 가져와 num1에 대입해줌
		
		System.out.print("정수 입력 2 :");
		int num2 = sc.nextInt(); // 다음에 입력된 정수를 가져와 num2에 대입해줌
		...
		// **************************************
		// -next() / nextInt() / nexDouble() 등을 작성한 이후
		// nextLine() 을 작성해야되는 상황이 오면
		// nextLine() 을 미리 한번 작성하고 사용하기
		
		sc.nextLine(); // next() / nextInt() / nexDouble() 뒤에 남아있는 버퍼값을 Line안에 넣어줌
		// 그런데 버퍼값은 우리가 눈으로 보거나 저장을 해야할 이유가 없기 때문에
		// 앞에 자료형이나 변수명을 사용해서 버퍼를 넣어 저장해야할 이유가 없어 sc,nextLine()으로만 사용
		...
		// Scanner를 종료하는 close()를 사용해서 닫아줌
		sc.close();
